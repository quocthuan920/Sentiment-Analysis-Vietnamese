...
report1 = metrics.classification_report(y_test, ynb_pred, labels=['0', '1', '2'], digits=3, zero_division=0)
print('Report: ',report1)
from sklearn.metrics import accuracy_score, f1_score, confusion_matrix

print("Accuracy: {:.2f}%".format(accuracy_score(y_test, ynb_pred) * 100))
print("\nWeighted F1 Score: {:.2f}".format(f1_score(y_test, ynb_pred, average='weighted') * 100))
...
filename = 'models/BAYES_model_VSFC_Sentiments.sav'
...
input_test[['Sents', 'Sentiments']].to_csv('submits/BAYES_VSFC_Sentiments.csv',encoding="utf-8-sig", index=False)
